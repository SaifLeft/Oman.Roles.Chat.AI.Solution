...
namespace Mohami.AI.Maui.Shared.Components
{
    public partial class GoogleMap : MudComponentBase, IAsyncDisposable
    {
        ...
        public async Task OnMapClick(GoogleMapClickEventArgs args)
        {
            await MapClick.InvokeAsync(args);
        }

        [JSInvokable("GoogleMap.OnMarkerClick")]
        public async Task OnMarkerClick(GoogleMapMarker marker)
        {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                 ...
                 await _jsRuntime.InvokeVoidAsync("Mohami.AI.Maui.createMap", Element, Reference, UniqueID, ApiKey, Zoom, Center,
                     data.Select(m => new { m.Title, m.Label, m.Position }));
            }
            else
            {
                await _jsRuntime.InvokeVoidAsync("Mohami.AI.Maui.updateMap", UniqueID, Zoom, Center,
                             data.Select(m => new { m.Title, m.Label, m.Position }));
            }
        }
        ...
        public async Task MoveToCenter()
        {
            ...
            await _jsRuntime.InvokeVoidAsync("Mohami.AI.Maui.updateMap", UniqueID, Zoom, Center,
                            data.Select(m => new { m.Title, m.Label, m.Position }));
            //await _jsRuntime.InvokeVoidAsync("Mohami.AI.Maui.moveToCenter", UniqueID, Center);
        }
        public ValueTask DisposeAsync()
        {
            return _jsRuntime.InvokeVoidAsync("Mohami.AI.Maui.destroyMap", UniqueID);
        }
