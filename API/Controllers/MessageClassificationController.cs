...
namespace API.Controllers
{
    ...
    public class MessageClassificationController : ControllerBase
    {
        ...
        /// <param name="language">اللغة</param>
        /// <returns>معلومات التصنيف</returns>
        [HttpPost("setCategory/{messageId}/{categoryId}")]
        ...
        public async Task<IActionResult> GetCategorySummary([FromQuery] AnalyticsPeriodQuery query)
        {
            try
            {
                var userId = User.Claims.FirstOrDefault(c => c.Type == CustomClaimTypes.UserId)?.Value;
